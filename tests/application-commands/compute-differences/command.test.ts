...
import { getCommandDifferences as getCommandDifferencesRaw } from "@/application-commands/compute-differences/command.ts";

function getCommandDifferences(
	...args: Parameters<typeof getCommandDifferencesRaw>
) {
...
describe("Compute differences for provided application commands", () => {
	...
	test("GIVEN a command WHEN no options and one with an empty options array THEN do not throw an error", () => {
		...
		expect(() =>
			getCommandDifferences(command1, command2, false),
		).not.toThrow();
		expect(() =>
			getCommandDifferences(command2, command1, false),
		).not.toThrow();
	});
