...
import { Lexer, Parser } from "@utilities/lexure/index.ts";

const parse = (testString: string) =>
	new Parser(
		new FlagUnorderedStrategy({
			flags: ["f", "hello"],
			options: ["o", "option"],
		}),
	).run(
...
describe("Flag parsing strategy", () => {
	test("GIVEN typeof FlagUnorderedStrategy THEN returns function", () => {
	...
	test("GIVEN option with multiple occurrences inside text THEN returns option with multiple values", () => {
