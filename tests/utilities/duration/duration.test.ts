...
import { Duration, Time } from "@utilities/duration/index.ts";

describe("Duration", () => {
	describe("units", () => {
		function expectDurationUnits(
			duration: Duration,
			units: Partial<Record<keyof Duration, number>> = {},
		) {
		...
		test.each([
			...
			"0mo",
			"0yr",
		])("GIVEN %s THEN shows 0ms", (pattern) => {
		...
		test.each(["a microsecond", "1us", "1μs"])(
			"GIVEN %s THEN shows 1μs",
			(pattern) => {
				const duration = new Duration(pattern);
		...
		test.each(["a millisecond", "1ms"])(
			"GIVEN %s THEN shows 1ms",
			(pattern) => {
				const duration = new Duration(pattern);
	...
	test("GIVEN duration with offset, THEN dateFrom is valid", () => {
		...
		const date = new Date();
		expect(duration.dateFrom(date)).toEqual(
