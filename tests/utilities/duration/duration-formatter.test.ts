...
import { DurationFormatter, Time } from "@utilities/duration/index.ts";
import type { DurationFormatSeparators } from "@types";
...
function durationSeparatorImpl(
	time: number,
	separators: DurationFormatSeparators,
) {
...
describe("FriendlyDuration", () => {
	...
	test("GIVEN 1 day, 3 hours and 2 minutes w/o precision THEN shows 1 day 3 hours and 2 minutes", () => {
		expect(durationImpl(Time.Day + Time.Hour * 3 + Time.Minute * 2)).toEqual(
			"1 day 3 hours 2 minutes",
		);
	});

	test("GIVEN 1 day, 3 hours and 2 minutes w/ precision of 2 THEN shows 1 day and 3 hours", () => {
		expect(durationImpl(Time.Day + Time.Hour * 3 + Time.Minute * 2, 2)).toEqual(
			"1 day 3 hours",
		);
	});
	...
	test("GIVEN 1 day, 3 hours and 2 minutes w/ final separator THEN shows 1 day 3 hours and 2 minutes", () => {
		expect(
			durationSeparatorImpl(Time.Day + Time.Hour * 3 + Time.Minute * 2, {
				final: " and ",
			}),
		).toEqual("1 day 3 hours and 2 minutes");
	});
	...
	test("GIVEN durations higher than 1 THEN shows plurals", () => {
		expect(durationImpl(Time.Day * 2 + Time.Hour * 2)).toEqual(
			"2 days 2 hours",
		);
	});
});
