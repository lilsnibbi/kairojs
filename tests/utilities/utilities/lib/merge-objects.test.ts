...
import { mergeObjects } from "@utilities/utilities/index.ts";
...
describe("mergeObjects", () => {
	...
	test("GIVEN deep-replace THEN returns expected", () => {
		const source = { a: { i: 4 } };
		const target = { a: 0 };
		expect(mergeObjects(target, source) as unknown).toEqual({ a: { i: 4 } });
	});
	...
	test("GIVEN deep-type-mismatch THEN returns expected", () => {
		const source = { a: 0 };
		const target = { a: { b: 1 } };
		expect(mergeObjects(target, source) as unknown).toEqual({ a: { b: 1 } });
	});
});
