...
import { deepClone } from "@utilities/utilities/index.ts";
...
describe("deepClone", () => {
	...
	] as const)("Given %s THEN returns same", (_, TypedArray) => {
	...
	test("GIVEN nested array THEN returns new clone", () => {
		expect.assertions(4);
		const source: [number, number, number, (number | number[])[]] = [
			1,
			2,
			3,
			[4, 5, [6, 7, 8]],
