...
import { BitField } from "@utilities/bitField/index.ts";
...
describe("BitField", () => {
	...
	describe("constructor", () => {
		...
		test.each([true, 42, 10n, "foo", undefined])(
			"GIVEN a non-object THEN throws TypeError",
			(value) => {
		...
		test.each([true, {}, undefined, null, "foo"])(
			"GIVEN a non-numeric value THEN throws TypeError",
			(value) => {
		...
		describe("number", () => {
			test.each([true, {}, 42n, undefined, null, "foo"])(
				"GIVEN a non-number sibling value THEN throws TypeError",
				(value) => {
			...
			test.each([1.2, 4294967296])(
				"GIVEN an unsafe bitfield value THEN throws RangeError",
				(value) => {
					const given = () => new BitField({ Read: value });
					const expected =
						'The property "Read" does not resolve to a safe bitfield value';
			...
			test.each([0, -8])(
				"GIVEN a non-positive value THEN throws RangeError",
				(value) => {
					const given = () => new BitField({ Read: value });
					const expected =
						'The property "Read" resolves to a non-positive value';
		...
		describe("bigint", () => {
			test.each([true, {}, 42, undefined, null, "foo"])(
				"GIVEN a non-bigint sibling value THEN throws TypeError",
				(value) => {
	...
	describe("prototype", () => {
		...
		describe("resolve", () => {
			...
			// Written as a loop rather than `test.each` because Bun formats `%o` titles through
			// `JSON.stringify`, which cannot serialise the `4n` case.
			for (const value of [4n, undefined, true]) {
				test(`GIVEN ${String(value)} THEN throws TypeError`, () => {
					const given = () => bitfield.resolve(value as any);
					const expected =
						'Received a value that is not either type "string", type "number", or an Array';

					expect(given).toThrowError(expected);
				});
			}

			test.each(["Execute", "Foo"])(
				"GIVEN %s THEN throws RangeError",
				(value) => {
					const given = () => bitfield.resolve(value as any);
					const expected =
			...
			test.each([null, {}])(
				"GIVEN non-Array object THEN throws TypeError",
				(value) => {
					const given = () => bitfield.resolve(value as any);
		...
		describe("union", () => {
			...
			test("GIVEN Read, Write, Delete THEN returns Read | Write | Delete", () => {
				expect<number>(bitfield.union("Read", "Write", "Delete")).toBe(
					NumberFlags.Read | NumberFlags.Write | NumberFlags.Delete,
				);
			});
		});
		...
		describe("symmetricDifference", () => {
			test("GIVEN A ⋂ B = ∅ THEN returns A ∪ B", () => {
				const given = bitfield.symmetricDifference(
					["Read", "Write"],
					["Edit", "Delete"],
				);
				const expected =
		...
		describe("toArray", () => {
			test("GIVEN ∅ THEN returns empty array", () => {
				...
				expect<string[]>(given).toEqual(expected as unknown as string[]);
			});
		...
		describe("toValues", () => {
			test("GIVEN ∅ THEN returns empty iterator", () => {
				...
				expect<number[]>(given).toEqual(expected as unknown as number[]);
			});
		...
		describe("toEntries", () => {
			test("GIVEN ∅ THEN returns empty iterator", () => {
				...
				expect<[string, number][]>(given).toEqual(
					expected as unknown as [string, number][],
				);
			});
