...
import { reduce } from "@utilities/iterator-utilities/index.ts";
...
describe("reduce", () => {
	...
	test("GIVEN empty iterable and no initial value THEN throws TypeError", () => {
		...
		expect(() => reduce(iterable, callbackFn)).toThrow(
			new TypeError("Reduce of empty iterator with no initial value"),
		);
