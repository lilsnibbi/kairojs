...
import { range } from "@utilities/iterator-utilities/index.ts";
...
describe("range", () => {
	...
	test("GIVEN start, end, and zero step THEN throws RangeError", () => {
		...
		expect(() => [...range(start, end, step)]).toThrow(
			new RangeError("Step cannot be zero"),
		);
	});
	...
	test("GIVEN start greater than end and positive step THEN throws RangeError", () => {
		...
		const step = 1;
		expect(() => [...range(start, end, step)]).toThrow(
			new RangeError("Start must be less than end when step is positive"),
		);
	});
	...
	test("GIVEN start less than end and negative step THEN throws RangeError", () => {
		...
		const step = -1;
		expect(() => [...range(start, end, step)]).toThrow(
			new RangeError("Start must be greater than end when step is negative"),
		);
	});
