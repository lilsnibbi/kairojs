...
import { from } from "@utilities/iterator-utilities/index.ts";
...
describe("from", () => {
	...
	test("GIVEN an object with non-iterable values THEN throws TypeError", () => {
		...
		expect(() => from(obj)).toThrow(
			new TypeError("[object Object] cannot be converted to an iterable"),
		);
	});
	...
	test("GIVEN a null object THEN throws TypeError", () => {
		...
		// @ts-expect-error Testing invalid input
		expect(() => from(obj)).toThrow(
			new TypeError("null cannot be converted to an iterable"),
		);
	});
	...
	test("GIVEN an invalid input THEN throws TypeError", () => {
		...
		expect(() => from(1)).toThrow(
			new TypeError("1 cannot be converted to an iterable"),
		);
	});
});
