...
import { chunk } from "@utilities/iterator-utilities/index.ts";
...
describe("chunk", () => {
	...
	test("GIVEN iterable and chunk size zero THEN throws RangeError", () => {
		const iterable = [1, 2, 3];
		const chunkSize = 0;
		expect(() => [...chunk(iterable, chunkSize)]).toThrowError(
			new RangeError("0 must be a positive number"),
		);
	});
	...
	test("GIVEN iterable and negative chunk size THEN throws RangeError", () => {
		...
		const chunkSize = -1;
		expect(() => [...chunk(iterable, chunkSize)]).toThrowError(
			new RangeError("-1 must be a positive number"),
		);
	});
	...
	test("GIVEN iterable and Infinity chunk size THEN throws RangeError", () => {
		...
		expect(() => [...chunk(iterable, chunkSize)]).toThrowError(
			new RangeError("+Infinity cannot be represented as an integer"),
		);
	});
