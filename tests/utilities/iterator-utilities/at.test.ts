...
import { at } from "@utilities/iterator-utilities/index.ts";
...
describe("at", () => {
	...
	test("GIVEN iterable and negative index THEN throws RangeError", () => {
		const iterable = [1, 2, 3, 4, 5];
		const index = -1;
		expect(() => at(iterable, index)).toThrowError(
			new RangeError("-1 must be a non-negative number"),
		);
	});
	...
	test("GIVEN iterable and -Infinite index THEN throws RangeError", () => {
		...
		const index = -Infinity;
		expect(() => at(iterable, index)).toThrowError(
			new RangeError("-Infinity must be a non-negative number"),
		);
	});
});
