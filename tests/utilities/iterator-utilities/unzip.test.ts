...
import { unzip } from "@utilities/iterator-utilities/index.ts";
...
describe("unzip", () => {
	...
	test("GIVEN iterable with empty array THEN throws an error", () => {
		...
		expect(() => unzip(iterable)).toThrowError(
			"Cannot unzip an empty iterable",
		);
	});
	...
	test("GIVEN iterable with non-array value THEN throws an error", () => {
		...
		expect(() => unzip(iterable)).toThrowError(
			"Cannot unzip an iterable that does not yield an array",
		);
	});
