...
import { sum } from "@utilities/iterator-utilities/index.ts";
...
describe("sum", () => {
	...
	test("GIVEN iterable with strings THEN throws RangeError", () => {
		...
		expect(() => sum(iterable)).toThrow(
			new RangeError("a must be a non-NaN number"),
		);
	});

	test("GIVEN iterable with NaN THEN throws RangeError", () => {
		const iterable = [1, 2, 3, NaN, 5];
		expect(() => sum(iterable)).toThrow(
			new RangeError("NaN must be a non-NaN number"),
		);
	});
	...
	test("GIVEN iterable with bigints THEN throws TypeError", () => {
		...
		// @ts-expect-error Testing invalid input
		expect(() => sum(iterable)).toThrow(
			new TypeError("Cannot convert a BigInt value to a number"),
		);
	});
	...
	test("GIVEN iterable with objects THEN throws RangeError", () => {
		const iterable = [{ foo: "bar" }, { hello: "world" }, { bax: "qux" }];
		// @ts-expect-error Testing invalid input
		expect(() => sum(iterable)).toThrow(
			new RangeError("[object Object] must be a non-NaN number"),
		);
	});
	...
	test("GIVEN iterable with undefined THEN throws TypeError", () => {
		...
		// @ts-expect-error Testing invalid input
		expect(() => sum(iterable)).toThrow(
			new TypeError("Cannot convert an undefined value to a number"),
		);
	});
});
