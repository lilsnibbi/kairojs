...
describe("Schema", () => {
	...
	describe("serialization", () => {
		test("GIVEN a schema with a boolean property THEN it serializes correctly (serialize)", () => {
			...
			expectTypeOf(value).toExtend<{ a: boolean }>();
			expect(value).toEqual({ a: true, b: 15234 });
		});
	...
	describe("exceptions", () => {
		test("GIVEN a schema with a duplicate property THEN it throws", () => {
			const schema = new Schema(1).boolean("a");
			expect(() => schema.boolean("a")).toThrowError(
				'Schema with id 1 already has a property with name "a"',
			);
		});
		...
		test("GIVEN a schema with a non-existent property name THEN it throws", () => {
			...
			expect(() => schema.get("b")).toThrowError(
				'Schema with id 1 does not have a property with name "b"',
			);
		});
	});
});
