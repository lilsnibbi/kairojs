...
import type { DuplexBuffer } from "@types";
import {
	type Float64Type,
	Schema,
	SchemaStore,
...
describe("SchemaStore", () => {
	...
	test("GIVEN a SchemaStore with a Schema THEN it has the correct properties and types", () => {
		...
		expectTypeOf<SchemaStore<{ 2: Schema<2, { height: typeof Float64Type }> }>>(
			store,
		);
	...
	describe("serialization", () => {
		test("GIVEN a schema and a value THEN it serializes and deserializes the buffer correctly", () => {
			const store = new SchemaStore(10).add(
				new Schema(2).string("name").float64("height"),
			);

			const buffer = store.serializeRaw(2, { name: "Mario", height: 1.8 });
			const deserialized = store.deserialize(buffer);
			expectTypeOf(deserialized).toExtend<{
				id: 2;
				data: { height: number };
			}>();
			expect(deserialized).toEqual({
				id: 2,
				data: { name: "Mario", height: 1.8 },
			});
		...
		test("GIVEN a schema with a constant THEN it serializes and deserializes the buffer correctly", () => {
			const store = new SchemaStore(10).add(
				new Schema(2).constant("runner", "bun"),
			);

			const buffer = store.serialize(2, {});
			const deserialized = store.deserialize(buffer);
			expect<{ id: 2; data: { runner: "bun" } }>(deserialized).toEqual({
				id: 2,
				data: { runner: "bun" },
			});
	...
	describe("exceptions", () => {
		test("GIVEN a schema ID that already exists THEN it throws", () => {
			const schema = new Schema(2).float64("height");
			const store = new SchemaStore().add(schema);
			expect(() => store.add(schema)).toThrowError(
				"Schema with id 2 already exists",
			);
		});
		...
		test("GIVEN a schema ID that does not exist THEN it throws", () => {
			...
			expect(() => store.get(2)).toThrowError(
				"Schema with id 2 does not exist",
			);
		});

		test.each(["", UnalignedUint16Array.from("")])(
			"GIVEN an empty value to `getIdentifier` THEN it throws",
			(value) => {
				const store = new SchemaStore();
				expect(() => store.getIdentifier(value)).toThrowError(
