...
import { UnalignedUint16Array } from "@utilities/string-store/index.ts";
...
describe("UnalignedUint16Array", () => {
	...
	test("GIVEN an instance with a 64-bit big integer written THEN it has the correct properties", () => {
		...
		expect(buffer.toArray()).toEqual(
			new Uint16Array([65535, 65535, 65535, 65535]),
		);
	...
	test("GIVEN an instance from invalid codepoints THEN it throws", () => {
		expect(() => UnalignedUint16Array.from("invalid input")).toThrowError(
			`Codepoint ${"i".codePointAt(0)} is out of range`,
		);
	});
});
