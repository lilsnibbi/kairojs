...
import {
	isArrayLike,
	isValidLength,
} from "@utilities/string-store/lib/shared/common.ts";

describe("isArrayLike", () => {
	test.each([undefined, null, 42, 100n, () => {}, Symbol("foo"), "bar"])(
		"GIVEN a non-object or null value THEN it returns false",
		(value) => {
	...
	test.each([{}, { length: "42" }])(
		"GIVEN an object without a numeric length property THEN it returns false",
		(value) => {
			expect(isArrayLike(value)).toBe(false);
		},
	);
...
describe("isValidLength", () => {
	test.each([undefined, null, 4.2, 100n, () => {}, Symbol("foo"), "bar"])(
		"GIVEN a non-safe integer THEN it returns false",
		(value) => {
	...
	test.each([
		...
		-Infinity,
		NaN,
	])("GIVEN an out-of-range length THEN it returns false", (value) => {
		expect(isValidLength(value)).toBe(false);
	});
});
