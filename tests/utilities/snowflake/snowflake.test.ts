...
import { Snowflake } from "@utilities/snowflake/index.ts";
import type { DeconstructedSnowflake } from "@types";
...
describe("Snowflake", () => {
	beforeAll(() => {
		setSystemTime(new Date("2020-01-01T00:00:00.000+00:00"));
	});
	...
	describe("generate", () => {
		...
		test("GIVEN timestamp as NaN THEN returns error", () => {
			...
			const snowflake = new Snowflake(sampleEpoch);
			expect(() => snowflake.generate({ timestamp: NaN })).toThrowError(
				bigIntNaNErrorMessage,
			);
		});
		...
		test("GIVEN timestamp as Date and increment much higher than 4095n THEN returns predefined snowflake", () => {
			const testId = "5000";
			const snowflake = new Snowflake(sampleEpoch);
		...
		/**
		 * The original suite reached into the instance's increment counter through the symbol it was
		 * keyed by. The rewrite stores it in a `#private` field, which nothing outside the class can
		 * observe or assign, so the same wrap-around behaviour is driven and observed purely through
		 * `generate` instead: the counter is walked up to the boundary by repeated calls, and the value
		 * it holds afterwards is read back from the next snowflake it produces.
		 */
		describe("increment overrides", () => {
			test("GIVEN near-limit THEN it reaches limit", () => {
				const snowflake = new Snowflake(sampleEpoch);
				for (let index = 0; index < 4094; index++) snowflake.generate();

				const snow = snowflake.generate();

				expect(snow.toString()).toBe("8190");
				// The counter now holds 4095, which the next snowflake reports:
				expect(snowflake.generate().toString()).toBe("8191");
			});

			test("GIVEN limit THEN it cycles to 0", () => {
				const snowflake = new Snowflake(sampleEpoch);
				for (let index = 0; index < 4095; index++) snowflake.generate();

				const snow = snowflake.generate();

				expect(snow.toString()).toBe("8191");
				// The counter cycled back to 0, which the next snowflake reports:
				expect(snowflake.generate().toString()).toBe("4096");
			});

			test("GIVEN over-limit THEN it is masked and leaves the counter untouched", () => {
				const snowflake = new Snowflake(sampleEpoch);
				const snow = snowflake.generate({ increment: 4096n });

				expect(snow.toString()).toBe("4096");
				// An explicit increment never advances the counter, which still holds 0:
				expect(snowflake.generate().toString()).toBe("4096");
			});

			test("GIVEN under-limit THEN it is masked and leaves the counter untouched", () => {
				const snowflake = new Snowflake(sampleEpoch);
				const snow = snowflake.generate({ increment: -1n });
	...
	describe.each([
		...
		(value: bigint) => string | bigint,
		(value: bigint) => string | bigint,
	][])("compare", (ctorA, ctorB) => {
		test.each<[string | bigint, string | bigint, -1 | 0 | 1]>([
		...
		])("GIVEN %s and %s THEN returns %d", (a, b, expected) => {
