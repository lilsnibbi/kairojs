...
import { resolveNumber } from "@/resolvers/number.ts";
...
describe("Number resolver", () => {
	...
	test("GIVEN a number smaller than minimum THEN returns error", () => {
		expect(resolveNumber("1.23", { minimum: 2 })).toEqual(
			Result.err(Identifiers.ArgumentNumberTooSmall),
		);
		expect(resolveNumber("1", { minimum: 2 })).toEqual(
			Result.err(Identifiers.ArgumentNumberTooSmall),
		);
	});

	test("GIVEN a number larger than maximum THEN returns error", () => {
		expect(resolveNumber("4.56", { maximum: 4 })).toEqual(
			Result.err(Identifiers.ArgumentNumberTooLarge),
		);
		expect(resolveNumber("5", { maximum: 4 })).toEqual(
			Result.err(Identifiers.ArgumentNumberTooLarge),
		);
	});

	test("GIVEN an invalid number THEN returns error", () => {
		expect(resolveNumber("hello")).toEqual(
			Result.err(Identifiers.ArgumentNumberError),
		);
	});
});
