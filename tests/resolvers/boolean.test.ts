...
import { resolveBoolean } from "@/resolvers/boolean.ts";
...
describe("Boolean resolver", () => {
	...
	test("GIVEN a truthy value with custom ones THEN returns true", () => {
		...
		expect(resolveBoolean("yup", { truths: ["yay", "yup", "yop"] })).toEqual(
			Result.ok(true),
		);
	});

	test("GIVEN a falsy value with custom ones THEN returns false", () => {
		expect(resolveBoolean("nah", { falses: ["nah"] })).toEqual(
			Result.ok(false),
		);
		expect(
			resolveBoolean("nope", { falses: ["nah", "nope", "noooo"] }),
		).toEqual(Result.ok(false));
	});

	test("GIVEN an invalid value THEN returns error", () => {
		expect(resolveBoolean("hello")).toEqual(
			Result.err(Identifiers.ArgumentBooleanError),
		);
		expect(
			resolveBoolean("world", { truths: ["nah", "nope", "noooo"] }),
		).toEqual(Result.err(Identifiers.ArgumentBooleanError));
	});
});
