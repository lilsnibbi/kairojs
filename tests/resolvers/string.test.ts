...
import { resolveString } from "@/resolvers/string.ts";
...
describe("String resolver", () => {
	...
	test("GIVEN a valid string with maximum THEN returns it", () => {
	...
	test("GIVEN a string shorter than minimum THEN returns error", () => {
		expect(resolveString("hello", { minimum: 10 })).toEqual(
			Result.err(Identifiers.ArgumentStringTooShort),
		);
		expect(resolveString("100", { minimum: 10 })).toEqual(
			Result.err(Identifiers.ArgumentStringTooShort),
		);
	});

	test("GIVEN a string longer than maximum THEN returns error", () => {
		expect(resolveString("hello", { maximum: 2 })).toEqual(
			Result.err(Identifiers.ArgumentStringTooLong),
		);
		expect(resolveString("100", { maximum: 2 })).toEqual(
			Result.err(Identifiers.ArgumentStringTooLong),
		);
	});
});
