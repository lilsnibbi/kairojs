...
import { resolveDate } from "@/resolvers/date.ts";
...
describe("Date resolver", () => {
	test("GIVEN a valid date-time THEN returns the associated timestamp", () => {
		expect(resolveDate(DATE_2020_PLAIN_STRING)).toEqual(Result.ok(DATE_2020));
	});

	test("GIVEN a valid date-time with minimum THEN returns the associated timestamp", () => {
		expect(resolveDate(DATE_2022_PLAIN_STRING, MINIMUM)).toEqual(
			Result.ok(DATE_2022),
		);
	});

	test("GIVEN a valid date-time with maximum THEN returns the associated timestamp", () => {
		expect(resolveDate(DATE_2018_PLAIN_STRING, MAXIMUM)).toEqual(
			Result.ok(DATE_2018),
		);
	});

	test("GIVEN a date-time before minimum THEN returns error", () => {
		expect(resolveDate(DATE_2018_PLAIN_STRING, MINIMUM)).toEqual(
			Result.err(Identifiers.ArgumentDateTooEarly),
		);
	});

	test("GIVEN a date-time beyond maximum THEN returns error", () => {
		expect(resolveDate(DATE_2022_PLAIN_STRING, MAXIMUM)).toEqual(
			Result.err(Identifiers.ArgumentDateTooFar),
		);
	});

	test("GIVEN an invalid date THEN returns error", () => {
		expect(resolveDate("hello")).toEqual(
			Result.err(Identifiers.ArgumentDateError),
		);
	});
});
