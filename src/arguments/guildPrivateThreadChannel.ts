...
import { resolveGuildPrivateThreadChannel } from "@/resolvers/index.ts";
import { Argument } from "@/structures/argument.ts";

/**
 * Parses a private thread of the guild the command was invoked in.
 *
 ...
 * @since 1.0.0
 */
export class CoreGuildPrivateThreadChannelArgument extends Argument<ThreadChannel> {
	...
	public run(
		...
		return resolveGuildPrivateThreadChannel(parameter, guild).mapErrInto(
			(identifier) =>
				this.error({
					parameter,
					identifier,
					message:
						"The given argument did not resolve to a valid private thread.",
...
void container.stores.loadPiece({
	...
	piece: CoreGuildPrivateThreadChannelArgument,
	store: "arguments",
});
