...
import { resolveGuildPublicThreadChannel } from "@/resolvers/index.ts";
import { Argument } from "@/structures/argument.ts";

/**
 * Parses a public thread of the guild the command was invoked in.
 *
 ...
 * @since 1.0.0
 */
export class CoreGuildPublicThreadChannelArgument extends Argument<ThreadChannel> {
	...
	public run(
		...
		return resolveGuildPublicThreadChannel(parameter, guild).mapErrInto(
			(identifier) =>
				this.error({
					parameter,
					identifier,
					message:
						"The given argument did not resolve to a valid public thread.",
...
void container.stores.loadPiece({
	...
	piece: CoreGuildPublicThreadChannelArgument,
	store: "arguments",
});
